...
$sql = mysql_query("SELECT * FROM pesan_pribadi_mini WHERE penerima='$initid' ORDER BY status_pesan='0' DESC LIMIT $posisi, $batas");
$no=$posisi;
while ($mes=mysql_fetch_array($sql))
{
	...
	echo 
	"<tr><td align=center valign=middle><input type=checkbox name=cek[] value=$mes[id_pesan] id=id$no></td>";
	$no++;	
...
$sql2=mysql_query("SELECT COUNT(penerima) as 'jum' FROM pesan_pribadi_mini WHERE penerima='$initid'");
