...
if (($rev == NULL or $rev == '%') and empty($lokasi_file))
{
	if ($revsoh==NULL)
	{
		echo "
		...
		echo "Klik <a href='editrevdos.php?key=$key&field=$field&urutan=$urutan&id_jud=$id_jud&halaman=$halaman&id_rev=$id_rev&kode=$kode&urutan_rep=$urutan_rep'>ini</a> untuk kembali ke menu Edit Post<br><br>";
		echo "<meta http-equiv='refresh' content='3;URL=editrevdos.php?key=$key&field=$field&urutan=$urutan&id_jud=$id_jud&halaman=$halaman&id_rev=$id_rev&kode=$kode&urutan_rep=$urutan_rep'></center></div>";
	}
	else
	{
		if ($del=="1")
...
else if (($rev == NULL or $rev == '%') and !empty($lokasi_file))
	{
		if ($jenis_rev=="")
		{
